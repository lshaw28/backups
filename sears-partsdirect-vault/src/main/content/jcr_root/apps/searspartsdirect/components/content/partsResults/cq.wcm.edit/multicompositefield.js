    ...
    constructor: function(config) {
        ...
        if(!config.readOnly) {
            items.push({
                ...
                "template": new CQ.Ext.Template('<span style="text-decoration:underline;float:right;margin-right:55%;cursor:pointer;">Add Item<button class="x-btn" type="{0}"></button></span>'),
                handler: function() {
                    list.addItem();
                    list.doLayout();
                    list.doLayout();
...
CQ.form.MtMultiCompositeField.Item = CQ.Ext.extend(CQ.Ext.Panel, {
    ...
    constructor: function(config) {
        ...
        config.panelConfig.cellCls = "sears-cq-multifield-itemct";
