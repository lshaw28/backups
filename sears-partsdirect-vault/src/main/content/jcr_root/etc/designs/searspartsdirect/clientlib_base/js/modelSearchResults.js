...
function fillDropdown(modelNumber, selectedValue, queryParam, dropDown) {
	...
	var urlName = "/bin/searspartsdirect/search/searchservlet?modelnumber="
			+ modelNumber + "&" + queryParam + "=" + selectedValue + "&flag=5";
	$
			.ajax({
				type : "GET",
				cache : false,
...
function modelSearchResults(modelNumber, pathTaken, flag, index, selectedValue) {
	...
	if (flag == 1) {
		// page change
		clearAll();
		$("#searchResultsUp").empty(); // SYW results should be empty when you
										// click on next/previous
		offset = index * 25;
		urlName = "/bin/searspartsdirect/search/searchservlet?modelnumber="
				+ modelNumber + "&offset=" + offset
				+ "&limit=25&sortType=revelence&flag=1";
	} else if (flag == 2) {
		// sort result
		clearAll();
		urlName = "/bin/searspartsdirect/search/searchservlet?modelnumber="
				+ modelNumber + "&offset=0&limit=25&flag=2";
		if (index == 0) {
		...
		clearAll();
		urlName = "/bin/searspartsdirect/search/searchservlet?modelnumber="
				+ modelNumber + "&offset=0&limit=25&sortType=revelence&flag=3";
		if (index != 0) {
			// if some other value is selected in dropdown, other than SELECT,
			// then only we will pass BRAND parameter.
		...
		clearAll();
		urlName = "/bin/searspartsdirect/search/searchservlet?modelnumber="
				+ modelNumber + "&offset=0&limit=25&sortType=revelence&flag=4";
		if (index != 0) {
			// if some other value is selected in dropdown, other than SELECT,
			// then only we will pass productType parameter.
			...
			.ajax({
				...
				success : function(data) {
					...
					if (jsonLength != 0) {
						...
						$("#searchCountTotal").append(
								parseInt(totalCount) + parseInt(sywCount));
						$("#searchCountSYW").append(sywCount);
						$("#searchCountDown").append(totalCount);
						$("#pageCountSYW").append(
								"1-" + sywCount + " of " + sywCount);
						...
						for ( var i = 2; i < jsonLength; i++) {
							var searchResults = jsonResponse[Object
									.keys(jsonResponse)[2]];
							searchResults = JSON.parse(searchResults);
							...
							for ( var j = 0; j < searchResults.length; j++) {
								...
								var resultDetail = searchResults[Object
										.keys(searchResults)[j]];

								if ((flag == 0 || flag == 1 || flag == 2
										|| flag == 3 || flag == 4)
										&& resultDetail.sywModel == false) {
										...
										$("#searchResultsDown")
												.append(
														""
																...
																+ "<p>Model <span><a onclick=\"showAllDiagram('"+resultDetail.modelNumber+"', '"+resultDetail.brandId+"', '"+resultDetail.categoryId+"', '"+resultDetail.brandName+"', '"+resultDetail.modelDescription+"')\">"
																...
																+ "<button class=\"new-btn new-btn-search\" onclick=\"showAllDiagram('"+resultDetail.modelNumber+"', '"+resultDetail.brandId+"', '"+resultDetail.categoryId+"', '"+resultDetail.brandName+"', '"+resultDetail.modelDescription+"')\">Shop Parts</button>"
																+ "</div></div>");
									}
									searchResultCount++;
								} else if ((flag == 0 || flag == 3 || flag == 4 || (flag == 1 && index == 0))
										&& resultDetail.sywModel == true) {
									/*
									 * Normal Result Flag 3 - Brand Selection
									 * Flag 4 - Product Selection Flag 1, index
									 * 0: Clicking on Previous and comes to
									 * first page
									 */
						...
						$(window.location).attr('href',
								'no-models-found.html?modelNum=' + modelNumber);
					}
...
function showAllDiagram(modelNumber, brandId, categoryId, brandName, modelDescription){
	window.location.href="/content/searspartsdirect/en/modelalldiagram.html?modelNumber="
	+ modelNumber
	+ "&brandId="
	+ brandId
	+ "&categoryId="
				...
				success : function(data) {
					...
					if (typeof divID === 'undefined') {
						...
						if (divID == 'productType') {
							...
							searchResults = jsonResponse[Object
									.keys(jsonResponse)[1]];
