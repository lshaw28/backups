...
public class PSDynamicTabsServlet extends SlingSafeMethodsServlet {
	...
	protected void doGet(SlingHttpServletRequest request,
		...
		if (StringUtils.isNotEmpty(modelNumber)&&StringUtils.isNotEmpty(brandId)&&StringUtils.isNotEmpty(categoryId)){
			try {
				getDynamicTabs(API_URL, jsonObject, modelNumber, brandId, categoryId);
	...
	private void getDynamicTabs(String API_URL, JSONObject jsonObject, String modelNumber, String brandId, String categoryId)
			throws JSONException, ValueFormatException, PathNotFoundException, RepositoryException, UnsupportedEncodingException {
		HttpClient client = new HttpClient();	 
			 API_URL = API_URL + URLEncoder.encode(modelNumber,"UTF-8")+"&brandId="+
					   URLEncoder.encode(brandId, "UTF-8") +"&categoryId="+URLEncoder.encode(categoryId, "UTF-8");
			 API_URL = StringUtils.replaceEach(API_URL,
		...
		try {
			...
			if (statusCode != HttpStatus.SC_OK) {
				...
				jsonObject.put("jsonDynamicTabs", method.getResponseBodyAsString());
			}
