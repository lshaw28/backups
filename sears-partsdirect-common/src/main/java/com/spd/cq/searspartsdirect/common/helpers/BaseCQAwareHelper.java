...
public class BaseCQAwareHelper {
	...
	public BaseCQAwareHelper(PageContext pageContext) {
		...
		pageProperties = (InheritanceValueMap) pageContext.findAttribute("pageProperties");
	...
	protected boolean isAuthoringMode() {
		return WCMMode.fromRequest(pageContext.getRequest()).equals( WCMMode.EDIT) || WCMMode.fromRequest(pageContext.getRequest()).equals( WCMMode.DESIGN);
	}

	protected String appendHtmlSelectorIfResource( String imgurl ) {
		Resource linkResource = getSlingRequest().getResourceResolver().getResource(imgurl);
